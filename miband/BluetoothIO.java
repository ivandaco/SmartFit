...
class BluetoothIO extends BluetoothGattCallback {
    ...
    NotifyListener stateChangedListener = null;

    public void connect(final Context context, final BluetoothDevice device, final ActionCallback callback) {
    ...
    public void enableNotifyListener(UUID serviceUUID, UUID characteristicId, NotifyListener listener) {
        if (null == gatt) {
            Log.e(TAG, "connect to miband first");
            return;
        }
        ...
        if(this.notifyListeners.containsKey(chara.getUuid()))
            return;
        ...
        this.notifyListeners.put(chara.getUuid(), listener);
    }

    public void disableNotifyListener(UUID serviceUUID, UUID characteristicId) {
        if (null == gatt) {
            Log.e(TAG, "connect to miband first");
            return;
        }
        ...
        if(this.notifyListeners.containsKey(chara.getUuid()))
            this.notifyListeners.remove(chara.getUuid());
    }
...
//                this.stateChangedListener.onNotify(null);
        }

        if (this.stateChangedListener != null) {
            ByteBuffer bf = ByteBuffer.allocate(4);
            bf.put((byte) (status & 0xff));
            bf.put((byte) (status >> 8 & 0xff));
            bf.put((byte) (status >> 16 & 0xff));
            bf.put((byte) (status >> 24 & 0xff));
            this.stateChangedListener.onNotify(bf.array());
        }
    }
...
//        Log.v("onCharacteristicChanged", characteristic.getUuid().toString() );
